...
class AuthController extends BaseController
{
    ...
    protected function getFacebookLoginData(
        ...
        if (!isset($token)) {
            ...
            $AuthUrl = $helper->getLoginUrl($p["redir_url"], $permissions);
            //Url de retorno onde iremos ter a validação do login. O domínio deverá ser configurado no seu app.
    ...
    protected function getGoogleLoginData(
        string $current_provider = null,
        string $section = "default",
        array $params = [
            "force_renew" => false,
            "redir_url" => "/google/login",
            "client_id" => "",
            "client_secret" => ""
        ]
    ) {
    ...
    public function attendeeLogin($key)
    {
        ...
        if (in_array($key, $this->allowedServices) || in_array($key, $this->allowedProviders)) {
            ...
            if (in_array($key, $this->allowedProviders)) {
                ...
                $provider = $this->getSessionLoginInfo("provider") ??
                    ($this->getSessionLoginInfo("connected", $section, "google")) ? "google" : (($this->getSessionLoginInfo("connected", $section, "facebook")) ? "facebook" : null);
